...
function listar(){
    ...
    local_store = localStorage.getItem("fila_espera");

    if(localStorage == null){
        resultado_espera.innerHTML +=  ` <p>Nenhum item cadastrado.</p>`
    } else {
        fila_espera = local_store.split(", ");
        for(let i = 0; i < fila_espera.length; i++){
        }
    }
...
function adicionar(nome, prioritario){
    ...
    localStorage.setItem("fila_espera", fila_espera.join(", "));
}
